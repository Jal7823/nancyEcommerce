...
def process_order(request):
    # creamos un pedido pasandole el usuario y modificando la propiedad completado
    ...
    # se pasa toda la informacion recopilada por la lista 'order_line'
    ...
    messages.success(
        request, 'Pedido creado con exito, mira nuestras novedades ')
    cart.clear()
...
def send_order_email(**kwargs):
    ...
    # se guarda en una variable para el cotexto de send mail
    plain_message = strip_tags(html_message)
    from_email = 'Departamento de ventas de Nancy'  # quien envia el correo

    to = kwargs.get('user_email',)
    print("🚀 ~ file: views.py ~ line 97 ~ to", to)
    # mailE = ('jal7823@gmail.com')  # para quien se envia

    # se pasa toda la info a la funcion que envia el pedido
